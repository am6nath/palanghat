...
const DEFAULT_CONFIG = {
  count: 60,
  colors: ["#000000", "#ffffff", "#888888"], // Crisp monochromatic black and white bubbles
...
class InstancedBubbles extends InstancedMesh {
...
function createBubblePit(canvasElement, config = {}) {
  ...
  const setupMesh = (conf) => {
    ...
    instancedMesh = new InstancedBubbles(engine.renderer, conf);
...
export function BubblePit({ className = "", followCursor = true, ...props }) {
  ...
  useEffect(() => {
    ...
    pitInstanceRef.current = createBubblePit(canvas, {
      followCursor,
      ...props,
    });
